...
namespace Plugin.NFC
{
	...
	public class NFCImplementation : INFC
	{
		...
		ITagInfo GetTagInfo(Tag tag, NdefMessage ndefMessage = null)
		{
            ...
            {
                if (nTag.Tech.Contains("android.nfc.tech.MifareUltralight")) //D&B
                {
                    try
                    {
                        ...
                        {
                            byte[] pageData = mifareUltralight.ReadPages(page);
                            Array.Copy(pageData, 0, data, page * 4, pageData.Length);
                        }
                ...
                else if (nTag.Tech.Contains("android.nfc.tech.NfcF")) //FeliCa
                {
                    try
                    {
                        var felica = NfcF.Get(tag);
                        felica.Connect();

                        nTag.SystemCode = felica.GetSystemCode();
                        nTag.Manufacturer = felica.GetManufacturer();
                ...
                else if (nTag.Tech.Contains("android.nfc.tech.IsoDep") && nTag.Tech.Contains("android.nfc.tech.NfcA"))
                {
